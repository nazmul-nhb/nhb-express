...
if (fs.existsSync(targetDir)) {
  const overwrite = await confirm({ message: `${projectName.toString()} already exists. Overwrite?` })
  if (!overwrite) {
...
const deps = ["bcrypt", "cookie-parser", "cors", "dotenv", "express", "jsonwebtoken", "mongoose", "multer", "nhb-toolbox", "zod", 'chalk@4.1.2']
const devDeps = ["@eslint/js", "@types/bcrypt", "@types/cookie-parser", "@types/cors", "@types/express", "@types/jsonwebtoken", "@types/node", "@typescript-eslint/eslint-plugin", "@typescript-eslint/parser", "eslint", "eslint-config-prettier", "eslint-plugin-prettier", "globals", "nhb-scripts", "nodemon", "prettier", "ts-node", "typescript", "typescript-eslint", "vercel"]
